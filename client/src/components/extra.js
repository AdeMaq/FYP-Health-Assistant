...
function PostureMap() {
    ...
    const takeSnapshot = (currentAngle, failedParts) => {
        ...
        const now = Date.now();
        if (now - lastSnapshotTime.current > 4000 && canvasRef.current && playerRef.current?.getCurrentTime) {
            ...
            // Create a specific mistake label based on identified failed parts
            ...
            setHistory((prev) => [
                ...prev,
                {
                    ...
                    status: currentAngle > (TARGET_ANGLE + 20) ? "Critical" : "Warning",
                    suggestion: currentAngle > 90 ? "Lift higher" : "Extend more",
    ...
    useEffect(() => {
        ...
        pose.onResults((results) => {
            ...
            // 3. Calibration Logic
            // Ensure shoulders and hips are visible before starting session
            ...
            if (!isCalibrated) {
                if (isBodyVisible) {
                    ...
                    // Draw a yellow guide skeleton during calibration
            ...
            // 4. Multi-Part Evaluation Logic
            const currentAngle = calculateAngle(lm[11], lm[13], lm[15]);

            const status = {
                FACE: lm[0].visibility > 0.5,
                ARMS: currentAngle > 60 && currentAngle < 140, // Correct arm extension range
                MIDDLE: Math.abs(lm[11].x - lm[23].x) < 0.15,  // Vertical alignment/lean
                LEGS: calculateAngle(lm[23], lm[25], lm[27]) > 70   // Hip/Knee angle
            };

            // Identify failed parts for the Snapshot/Table history
            const failedParts = [];
            ...
            // 5. Face Blur Logic
            // Blurs facial features if the exercise is body-focused
            ...
            // 6. Segmented Connection Drawing
            POSE_CONNECTIONS.forEach(([start, end]) => {
                let connectionColor = "#FFFFFF33"; // Default faint color

                // Determine color based on which segment the point belongs to and its status
                if (BODY_PARTS.FACE.includes(start))
                    connectionColor = status.FACE ? DEFAULT_COLORS.FACE : "#FF0000";
                else if (BODY_PARTS.ARMS_SHOULDER_NECK.includes(start))
                    connectionColor = status.ARMS ? DEFAULT_COLORS.ARMS_SHOULDER_NECK : "#FF0000";
                else if (BODY_PARTS.MIDDLE_ABDOMEN.includes(start))
                    connectionColor = status.MIDDLE ? DEFAULT_COLORS.MIDDLE_ABDOMEN : "#FF0000";
                else if (BODY_PARTS.HIPS_LEGS.includes(start))
                    connectionColor = status.LEGS ? DEFAULT_COLORS.HIPS_LEGS : "#FF0000";
            ...
            // 7. Individual Landmark Dot Drawing
        ...
        <div style={{ padding: "0px", backgroundColor: "#f8f9fa", minHeight: "100vh", fontFamily: "Arial" }}>
            ...
            <div style={styles.mainGrid}>
                <div style={styles.card}>
                    ...
                    <div style={styles.videoContainer}>
                        <iframe
                            ...
                            width="100%"
                            height="100%"
                            src={`https://www.youtube.com/embed/${videoId}?enablejsapi=1&mute=1&controls=1`}
                            frameBorder="0"
                ...
                <div style={styles.card}>
                    ...
                    <div
                        style={{ ...styles.videoContainer, cursor: "pointer" }}
                        onClick={toggleSync}
                    >
                        {!isPaused ? (
                            <>
                        ...
                        <table style={styles.matrixTable}>
                            <thead>
                                <tr style={{ backgroundColor: "#343a40", color: "#fff" }}>
                                    ...
                                    <th style={styles.th}>Detected Angle</th>
                            ...
                            <tbody>
                                {history.map((item, idx) => (
                                    <tr key={idx} style={styles.tr}>
                                        ...
                                        <td style={{ ...styles.td, color: "#dc3545" }}>+{item.deviation}°</td>
                                        <td style={{ ...styles.td, fontWeight: 'bold', color: item.status === "Critical" ? "#dc3545" : "#ffc107" }}>
                                            {item.status}
                                        </td>
                            ...
                            {history.map((item, idx) => (
                                <div key={idx} style={styles.comparisonCard}>
                                    <div style={styles.sideBySide}>
                                        <div
                                            style={{ ...styles.snapshotBox, cursor: "pointer" }}
                                            onClick={() => {
                                                playerRef.current?.seekTo(item.videoTimestamp);
                                                playerRef.current?.playVideo();
                                    ...
                                    <div style={styles.analysisFooter}>
                                        <strong>Logged At:</strong> {item.timestamp} |
                                        <strong> Video Time:</strong> {item.displayVideoTime} |
                                        <strong> Deviation:</strong> <span style={{ color: "red" }}>+{item.deviation}°</span>
                                    </div>
                                </div>
                            ))}
                        </div>
...
const styles = {
    ...
    overlay: {
        ...
        display: "flex", justifyContent: "center", alignItems: "center", color: "#fff", background: "rgba(0,0,0,0.5)"
    },
    playIcon: {
        fontSize: "60px", background: "rgba(40, 167, 69, 0.8)", width: "80px", height: "80px",
        display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", margin: "0 auto"
    },
    ...
    labelTag: {
        position: "absolute", top: "5px", left: "5px", backgroundColor: "#28a745", color: "#fff",
        padding: "3px 6px", borderRadius: "3px", fontSize: "9px", fontWeight: "bold", zIndex: 2
    },
    seekOverlay: {
        position: "absolute", bottom: 0, width: "100%", background: "rgba(0,0,0,0.6)",
        color: "#fff", fontSize: "10px", textAlign: "center", padding: "4px 0"
    },
